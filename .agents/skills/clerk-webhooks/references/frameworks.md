...
Each Clerk SDK package ships its own `verifyWebhook` adapter that reads the framework's native request type. Most read `CLERK_WEBHOOK_SIGNING_SECRET` automatically — Astro is the exception: `@clerk/astro/webhooks` requires passing `signingSecret` explicitly (see the Astro section below). Use the framework-specific import; do not roll your own with raw `svix`.
...
app.post('/api/webhooks', express.raw({ type: 'application/json' }), async (req, res) => {
  try {
    ...
    if (evt.type === 'user.created') {
      ...
      console.log(`New user: ${first_name} ${last_name} (${id})`)
    }
