...
export async function POST(req: NextRequest) {
  ...
  // Creates/updates use upserts (and deletes tolerate an already-missing row) so a
  // Svix retry replaying the same event doesn't fail or create a duplicate row.
  ...
  if (evt.type === 'user.deleted') {
    const { id } = evt.data
    await db.users.deleteMany({ where: { clerkId: id } }) // tolerant of an already-deleted row
  }
  ...
  if (evt.type === 'organizationMembership.deleted') {
    ...
    const orgId = organization.id
    const userId = public_user_data.user_id
    await db.teamMembers.deleteMany({ where: { orgId, userId } }) // tolerant of an already-deleted row
  }
...
export async function POST(req: NextRequest) {
  ...
  if (evt.type === 'user.created') {
    ...
    // email_addresses[0] is not guaranteed to be primary — match primary_email_address_id.
    const { email_addresses, primary_email_address_id, first_name, last_name } = evt.data
    const email = email_addresses.find((e) => e.id === primary_email_address_id)?.email_address
    const name = `${first_name ?? ''} ${last_name ?? ''}`.trim()

    // Step 4: Call Resend API to send welcome email (skip if the user has no email on file,
    // e.g. a phone-only account). Resend returns { data, error } instead of throwing.
    if (email) {
    ...
    await fetch(process.env.SLACK_WEBHOOK_URL!, {
      ...
      body: JSON.stringify({
        text: `New user signed up: ${name}${email ? ` (${email})` : ''}`,
      }),
    })
  }

  // For high-volume production apps, enqueue the email/Slack calls in a background
  // job or outbox instead of awaiting them inline here — that lets the webhook ack
  // immediately and avoids Svix retrying (and re-sending) the notifications if a
  // provider is slow. See "Webhook Reliability" below.
...
export async function POST(req: NextRequest) {
  ...
  if (evt.type === 'organizationMembership.created') {
    ...
    // Add to team_members table (the workspace itself is created once, in
    // organization.created above — not per member)
  ...
  if (evt.type === 'organizationMembership.deleted') {
    ...
    // Remove from team_members table (tolerant of an already-removed row)
    await db.team_members.deleteMany({
      where: { orgId, userId },
    })
  }
...
const {
  id,                          // Clerk user ID
  email_addresses,             // array; match id against primary_email_address_id — [0] is not guaranteed primary
