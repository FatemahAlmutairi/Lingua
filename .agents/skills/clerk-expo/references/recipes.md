...
if (signInResource.status === 'complete') {
  // session is active — navigate into the app
} else {
  // handle any additional requirement (e.g. needs_second_factor) the same way
  // as a regular signIn custom flow (see custom-flows.md)
}
```
