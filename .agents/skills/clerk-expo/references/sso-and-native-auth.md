...
const onPress = async () => {
  try {
    ...
    if (createdSessionId) {
      ...
      // Existing user has MFA enabled — route to your second-factor UI (see custom-flows.md)
    } else if (signIn?.status === 'needs_client_trust') {
      // Device isn't trusted yet — route to the device-trust verification step
    ...
    // No createdSessionId and none of the above → user cancelled; do nothing.
  } catch (err) {
